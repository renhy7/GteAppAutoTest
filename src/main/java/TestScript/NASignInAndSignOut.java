...
public class NASignInAndSignOut extends SupperTestClass {
	...
	public void NALgoin() {
		...
		try {
			...
			if(common.checkElementDisplays(driver, pages.StartupPage_OpenAccess, 5)){
				pages.StartupPage_OpenAccess.click();
			}
			
			Dailylog.logInfo("判断是否是首次启动："+(common.checkElementDisplays(driver, pages.Home_PersonInfo, 5)||common.checkElementDisplays(driver, pages.Login_LoginButton, 5)));
			
			if(!(common.checkElementDisplays(driver, pages.Home_PersonInfo, 5) || common.checkElementDisplays(driver, pages.Login_LoginButton, 5))){
				common.moveToLeft(driver, 4);
				pages.StartUsingAPP.click();
			}
			
			pages.Home_PersonInfo.click();
			if(common.checkElementDisplays(driver, pages.LeftNavigation_PhoneNumber, 5)){
			...
			common.assertToast("请输入图片中正确的内容");
			   ...
			   //验证短信验证码不能为空
			pages.Login_InputValidateCode.clear();
			pages.Login_InputValidateCode.sendKeys(common.getPhotoAndMessageValidate("cmn:verify:"+mobile+":imgcode",15));
			pages.Login_InputPassword.clear();
			pages.Login_LoginButton.click();
			...
			pages.Login_LoginButton.click();
			Dailylog.logInfo(common.getPhotoAndMessageValidate("cmn:verify:"+mobile+":code", 15));
			if(common.getPhotoAndMessageValidate("cmn:verify:"+mobile+":code", 15) == null){
			...
			pages.Login_InputPassword.clear();
			pages.Login_InputPassword.sendKeys(common.getPhotoAndMessageValidate("cmn:verify:"+mobile+":code", 15));
			...
			pages.Login_LoginButton.click();
			  //新手引导页面，第一步
			if(common.checkElementDisplays(driver, pages.NoviceGuide_NextClick, 5)){
			...
			Assert.assertTrue(common.checkElementDisplays(driver, pages.Home_PersonInfo, 10), "Login success……");
			...
			pages.LeftNavigation_More.click();
			pages.LeftNavigation_More_SignOut.click();
			Assert.assertTrue(common.checkElementExists(driver, pages.Login_LoginButton, 5), "Sign out success……");
