...
public class NABuyTicket extends SupperTestClass {
	...
	public void NABuyTicket() {    
		...
		String insertIntoRoutesPlanssql = "INSERT INTO `service_number_plans` "
				...
				+ CommonFunction.getNowTime() + "','22:15:00',"
				+ "'1','0','0.01','" + query + "','"
				+ CommonFunction.getNowDateFormatyyyyMMdd() + " 21:30:00','"
				+ CommonFunction.getNowDateFormatyyyyMMdd()
				+ " 22:15:00','1','1')";
		// 删除已经存在的计划
		...
		CommonFunction.Delete(deleteUserOrders);
		CommonFunction.Delete(deleteRoutesPlanssql); // 先删除线路计划
		CommonFunction.insertInto(insertIntoRoutesPlanssql); // 再进行插入线路计划

		try {
			Pages pages = new Pages(driver);
			CommonFunction.Login(driver, pages, mobile, defaultPassword);
			if (!CommonFunction.checkElementDisplays(driver, pages.Home_Title,
					5)) {
