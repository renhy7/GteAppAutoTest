...
public class NABuyTicket extends SupperTestClass {
	...
	public void  NABuyTicket(){
		...
		String  insertIntoRoutesPlanssql = "INSERT INTO `service_number_plans` "
				...
				+ " `last_date_time`, `create_id`, `part_dept_id`) VALUES('170','57','55','"+common.getNowDateFormatyyyyMMdd()+"',"
				+ "'21:30:00','16.8','','1','0','"+common.getNowTime()+"','"+common.getNowTime()+"','22:15:00',"
				+ "'1','0','0.01','"+query+"','"+common.getNowDateFormatyyyyMMdd()+" 21:30:00','"+common.getNowDateFormatyyyyMMdd()+" 22:15:00','1','1')";
		//删除已经存在的计划
		String  deleteRoutesPlanssql = "DELETE FROM `service_number_plans` WHERE service_date = '"+common.getNowDateFormatyyyyMMdd()+"' AND frequency_number = '"+query+"'";		
		//删除用户订单
		String deleteUserOrders = "DELETE FROM  `service_orders` WHERE passenger_id = (SELECT id FROM passenger WHERE mobile = '"+mobile+"')";
		...
		try{
			...
			WebElement BookSeat_SelectData = driver.findElement(By.xpath("//android.widget.TextView[@text=\""+common.getNowDateFormatMM()+"\"]"));
		    ...
		    if(common.checkElementDisplays(driver, pages.Pay_JudeGoBackAPP, 10)){
		    	Assert.assertTrue(true, "返回商家成功！！！！！！！");
		    }else{
		    	Assert.assertTrue(false, "返回商家失败，且没有跳转订单支付成功页面！！！！");
		    }
			if(common.checkElementDisplays(driver, pages.PaySuccess_ContinueBus, 10)){
				Assert.assertTrue(true, "继续乘车菜单加载成功！！！");
			}else{
				Assert.assertTrue(false,"继续乘车菜单加载失败……");
			}
			if(common.checkElementDisplays(driver, pages.PaySuccess_ViewOrders, 10)){
				Assert.assertTrue(true,"查看订单菜单加载成功！！！");
			}else{
				Assert.assertTrue(false,"查看订单菜单加载失败……");
			}
			if(common.checkElementDisplays(driver, pages.PaySuccess_JoinChat, 10)){
				Assert.assertTrue(true,"加入群聊菜单加载成功！！！");
			}else{
				Assert.assertTrue(false, "加入群聊菜单加载失败……");
			}
			if(common.checkElementDisplays(driver, pages.PaySuccess_ShareRedPacket, 10)){
			...
			if(common.checkElementDisplays(driver, pages.MyOrder_OrderStatus, 10)){
				Assert.assertTrue(pages.MyOrder_OrderStatus.getText().equals("已支付"), "支付成功后订单状态显示正确！！！");
			}else{
